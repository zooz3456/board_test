...
if(isset($_POST['search'])!="")
{
    ...
    $sql="select * from board1_img where {$field} LIKE '%{$search}%' limit {$page},{$per_page}";
}
else
{
...
if($rows)
{
    ...
    {
        ?>
        <tr onclick="location.href='img1.php?no=<?=$arr[$i]['no']?>'">
        <td><?=$arr[$i]['no']?></td>
        <td><?=$arr[$i]['writer']?></td>
...
?>        	<tr><td>
            <div class='search'>
                ...
                ?>
				<td>
                <input type="button" onclick="location.href='/dPdir.php'" value="글쓰기">
                </td>
				<?php
                }
                ?>
